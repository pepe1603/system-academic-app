...
const navItems = [
  { label: 'Inicio', to: '/portal', icon: 'i-lucide-home' },
    ...
    <nav class="border-b bg-background/80 backdrop-blur sticky top-0 z-40">
      <div class="container mx-auto px-6">
        <div class="flex items-center justify-between h-20">
          <NuxtLink to="/portal" class="flex items-center gap-3 font-semibold text-xl">
      ...
      <div class="relative z-20 container mx-auto px-6 h-full flex items-center">
        <div class="text-white max-w-3xl">
          ...
          <div v-if="activeBanners[0]?.linkUrl" class="mt-8">
            <a v-if="activeBanners[0].linkUrl.startsWith('/')" :href="activeBanners[0].linkUrl" class="inline-flex items-center justify-center px-8 py-4 text-lg font-medium rounded-lg bg-white text-primary hover:bg-white/90">
              Ver más <UIcon name="i-lucide-arrow-right" class="w-5 h-5 ml-2" />
            </a>
            <a v-else :href="activeBanners[0].linkUrl.startsWith('http') ? activeBanners[0].linkUrl : 'https://' + activeBanners[0].linkUrl" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center px-8 py-4 text-lg font-medium rounded-lg bg-white text-primary hover:bg-white/90">
              Ver más <UIcon name="i-lucide-arrow-right" class="w-5 h-5 ml-2" />
            </a>
          </div>
        </div>
    ...
    <section v-else class="bg-gradient-to-r from-primary to-primary/70 text-white py-20">
      <div class="container mx-auto px-6 text-center">
        ...
        <div class="mt-10 flex flex-wrap justify-center gap-6">
          <UButton size="xl" to="/portal/nosotros" variant="outline" class="border-white text-white hover:bg-white/10">
            Conócenos
          </UButton>
          <UButton size="xl" to="/portal/servicios" variant="outline" class="border-white text-white hover:bg-white/10">
            Servicios
          </UButton>
          ...
          <UCard v-for="event in events" :key="event.id" class="hover:shadow-xl transition-shadow">
            <template #header>
              <div class="flex items-start justify-between">
                ...
                <UBadge color="primary" size="lg">{{ event.eventDate }}</UBadge>
              </div>
            </template>
