export default defineEventHandler(async (event) => {
  ...
  console.log('[PROXY] 📝 PUT /api/users/' + id + '/ban')
  ...
  try {
    const response = await $fetch(`${config.public.apiBaseUrl}/users/${id}/ban`, {
      method: 'PUT',
    ...
    throw createError({
      ...
      message: err.data?.message || 'Error al banear usuario'
    })
  }
})