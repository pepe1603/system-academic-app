export default defineEventHandler(async (event) => {
  ...
  console.log('[PROXY] 📝 PUT /api/users/' + id + '/unlock')
  ...
  try {
    const response = await $fetch(`${config.public.apiBaseUrl}/users/${id}/unlock`, {
      method: 'PUT',
    ...
    throw createError({
      ...
      message: err.data?.message || 'Error al desbloquear usuario'
    })
  }
})