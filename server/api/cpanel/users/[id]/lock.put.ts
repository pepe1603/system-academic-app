export default defineEventHandler(async (event) => {
  ...
  console.log('[PROXY] 📝 PUT /api/users/' + id + '/lock')
  ...
  try {
    const response = await $fetch(`${config.public.apiBaseUrl}/users/${id}/lock`, {
      method: 'PUT',
    ...
    throw createError({
      ...
      message: err.data?.message || 'Error al bloquear usuario'
    })
  }
})